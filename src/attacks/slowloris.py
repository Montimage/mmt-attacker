...
class Slowloris(AttackBase):
    ...
    def add_arguments(self, parser: argparse.ArgumentParser) -> None:
        ...
        conn_group.add_argument(
            "--connections", type=int, default=150, help="Number of connections to keep open"
        )
        conn_group.add_argument(
            "--max-sockets", type=int, help="Maximum number of sockets to open"
        )
        conn_group.add_argument(
            "--socket-timeout", type=float, default=30.0, help="Socket timeout (seconds)"
        )
        conn_group.add_argument(
            "--connection-timeout", type=float, default=5.0, help="Connection timeout (seconds)"
        )
        conn_group.add_argument(
            "--keep-alive", action="store_true", help="Send keep-alive headers"
        )
        ...
        req_group.add_argument(
            "--header-size", type=int, default=15, help="Size of partial header chunk in bytes"
        )
        ...
        behavior_group.add_argument(
            "--timeout", type=float, default=30.0, help="Overall attack timeout (0 for no timeout)"
        )
        behavior_group.add_argument(
            "--ramp-up", type=int, help="Gradually increase connections over seconds"
        )
        behavior_group.add_argument(
            "--random-interval",
            action="store_true",
        ...
        behavior_group.add_argument(
            "--random-header", action="store_true", help="Use random header names"
        )
        ...
        monitor_group.add_argument(
            "--verify-vuln", action="store_true", help="Verify target vulnerability before attack"
        )
        monitor_group.add_argument(
