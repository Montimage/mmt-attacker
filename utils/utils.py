...
def exec_command(
    cmd: str, cwd: str | None = None, timeout: int | None = None, shell: bool = True
) -> str | None:
    ...
    try:
        ...
        logger.error(
            f"Command '{e.cmd}' failed with error code {e.returncode}: {e.output.decode('utf-8', errors='replace')}"
        )
        return None
...
def exec_command_safe(
    cmd_args: list[str], cwd: str | None = None, timeout: int | None = None
) -> str | None:
    ...
    try:
        ...
        logger.error(
            f"Command '{' '.join(cmd_args)}' failed with error code {e.returncode}: {e.output.decode('utf-8', errors='replace')}"
        )
        return None
    except subprocess.TimeoutExpired:
...
def get_application_path(app_name: str) -> str | None:
...
def write_json_file(file_path: str, data: dict | list, indent: int = 4) -> bool:
