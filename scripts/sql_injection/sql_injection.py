...
class SQLInjectionAttack:
    ...
    def _setup_browser(self) -> Browser:
        ...
        if self.verbose:
            logger.info(
                f"Browser configured with timeout={self.timeout}s, user-agent='{browser.addheaders[0][1]}'"
            )
            if self.proxy:
    ...
    def execute_attack(self, sql_attack_str: str) -> str | None:
        """
    ...
    def analyze_response(self, payload: str, response_content: str) -> None:
        ...
        for indicator in sql_error_indicators:
            if indicator.lower() in response_content.lower():
                logger.warning(
                    f"Potential SQL injection vulnerability detected! Error indicator: {indicator}"
                )
        ...
        if "admin" in response_content.lower() or "password" in response_content.lower():
            logger.warning("Potential sensitive data exposure detected in response!")
            logger.warning(f"Payload: {payload}")
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument("target_url", help="The URL of the target web application")

    parser.add_argument(
        "control_name", help="The name of the form control to inject SQL payloads into"
    )

    parser.add_argument(
        "attack_strings", nargs="*", help="SQL injection payloads to test (comma-separated)"
    )

    parser.add_argument(
        "-f", "--file", help="Path to a file containing SQL injection payloads (one per line)"
    )
    ...
    parser.add_argument(
        "-p", "--proxy", help="Use a proxy for requests (e.g., http://127.0.0.1:8080)"
    )
...
if __name__ == "__main__":
    ...
    if not attack_queries:
        default_query_file = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "sql-queries.txt"
        )
