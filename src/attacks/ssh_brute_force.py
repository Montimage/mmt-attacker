...
class SshBruteForce(AttackBase):
    ...
    def add_arguments(self, parser: argparse.ArgumentParser) -> None:
        ...
        target_group.add_argument(
            "--targets-file", help="File containing list of targets (one per line)"
        )
        ...
        auth_group.add_argument(
            "--usernames-file", help="File containing usernames (one per line)"
        )
        auth_group.add_argument("--password", help="Single password to try")
        auth_group.add_argument(
            "--passwords-file", help="File containing passwords (one per line)"
        )
        ...
        behavior_group.add_argument(
            "--delay", type=float, default=0.0, help="Delay between attempts (seconds)"
        )
        behavior_group.add_argument(
            "--max-attempts", type=int, help="Maximum login attempts per target"
        )
        behavior_group.add_argument(
            "--stop-on-success", action="store_true", help="Stop attacking after first success"
        )
        behavior_group.add_argument(
            "--shuffle", action="store_true", help="Randomize credential order"
        )
    ...
    def validate(self, args: argparse.Namespace) -> bool:
        ...
        if args.target and not (
            self.validator.validate_ip(args.target) or self.validator.validate_domain(args.target)
        ):
