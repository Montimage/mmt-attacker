...
class MacFlooding(AttackBase):
    ...
    def add_arguments(self, parser: argparse.ArgumentParser) -> None:
        ...
        parser.add_argument(
            "--count", type=int, default=1000, help="Number of frames (default: 1000)"
        )
        parser.add_argument(
            "--rate", type=int, default=100, help="Frames per second (default: 100)"
        )
    ...
    def run(self, args: argparse.Namespace) -> None:
        ...
        try:
            ...
            logger.info("Running mac-flooding attack")

            # Create and execute attack
            attack = attack_module.MACFloodingAttack(
                args.interface, args.count, args.rate, args.verbose
            )
            attack.execute()
