...
class BgpHijacking(AttackBase):
    ...
    def validate(self, args: argparse.Namespace) -> bool:
        ...
        if not self.validator.validate_network(args.prefix):
            logger.error(
                f"Invalid IP prefix/network: {args.prefix}. Must be in CIDR notation (e.g., 192.168.1.0/24)"
            )
            return False
    ...
    def run(self, args: argparse.Namespace) -> None:
        ...
        try:
            ...
            logger.info("Running bgp-hijacking attack")

            # Create and execute attack
            attack = attack_module.BGPHijackingAttack(args.prefix, args.as_number, args.verbose)
            attack.execute()
