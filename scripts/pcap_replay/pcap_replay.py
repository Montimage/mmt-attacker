...
class PCAPReplayAttack:
    ...
    def __init__(
        ...
        logger.info("Initialized PCAP replay attack simulation")
    ...
    def _print_summary(self, stats: dict[str, Any]) -> None:
        ...
        logger.info(
            f"Target:             {stats['target_ip']}{f':{stats["target_port"]}' if stats['target_port'] else ''}"
        )
...
""",
    ...
    parser.add_argument("-f", "--file", required=True, help="Path to the PCAP file to replay")

    parser.add_argument(
        "-t", "--target", required=True, help="Target IP address to send traffic to"
    )

    parser.add_argument(
        "-i", "--interface", required=True, help="Network interface to use for sending packets"
    )

    parser.add_argument("-p", "--port", type=int, help="Target port to send traffic to")

    parser.add_argument("-o", "--original-ip", help="Original destination IP in the PCAP file")

    parser.add_argument(
        "-g", "--original-port", type=int, help="Original destination port in the PCAP file"
    )
