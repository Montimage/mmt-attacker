...
class ArpSpoofing(AttackBase):
    ...
    def add_arguments(self, parser: argparse.ArgumentParser) -> None:
        ...
        target_group.add_argument(
            "--target-mac", help="Target MAC address (will be auto-discovered if not specified)"
        )
        target_group.add_argument("--gateway", required=True, help="Gateway IP address")
        target_group.add_argument(
            "--gateway-mac", help="Gateway MAC address (will be auto-discovered if not specified)"
        )
        ...
        net_group.add_argument(
            "--source-ip", help="Source IP address for spoofed packets (default: interface IP)"
        )
        net_group.add_argument(
            "--source-mac", help="Source MAC address for spoofed packets (default: interface MAC)"
        )

        # Attack behavior
        behavior_group = parser.add_argument_group("Attack Behavior")
        behavior_group.add_argument(
            "--interval", type=float, default=1.0, help="Time interval between packets (seconds)"
        )
        behavior_group.add_argument(
            "--count", type=int, default=0, help="Number of packets to send (0 for infinite)"
        )
        behavior_group.add_argument(
            "--aggressive",
            action="store_true",
            help="Send packets more frequently when target is active",
        )
        behavior_group.add_argument(
            "--bidirectional",
            action="store_true",
            help="Spoof both target->gateway and gateway->target",
        )
        behavior_group.add_argument(
            "--restore-on-exit", action="store_true", help="Restore ARP tables when attack ends"
        )
        ...
        monitor_group.add_argument(
            "--verify", action="store_true", help="Verify ARP tables are poisoned"
        )
        monitor_group.add_argument(
    ...
    def run(self, args: argparse.Namespace) -> None:
        ...
        try:
            ...
            if args.aggressive:
                logger.info(
                    "Running in aggressive mode - increased packet frequency when target is active"
                )
