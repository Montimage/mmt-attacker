...
try:
    from scapy.all import IP, TCP, RandIP, RandShort, conf, send
except ImportError:
...
class SYNFloodAttack:
    ...
    def __init__(
        ...
        logger.info("Initialized SYN flood attack simulation")
    ...
    def create_syn_packet(self, port: int) -> IP:
        ...
        if self.verbose:
            logger.debug(
                f"Created SYN packet: {src_ip}:{packet[TCP].sport} -> {self.target_ip}:{port}"
            )
