...
class SSHBruteForceAttack:
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized SSH brute force attack simulation against {target_ip}:{target_port}"
        )
        if self.verbose:
    ...
    def execute(self) -> dict[str, Any]:
        ...
        logger.info(
            f"Starting SSH brute force attack simulation against {self.target_ip}:{self.target_port}"
        )
        ...
        try:
            for password in self.passwords:
                ...
                if result == 0:
                    ...
                    logger.info(
                        f"[+] Success! Found valid credentials - Username: {self.username}, Password: {password}"
                    )
                    break
                    ...
                    logger.info(
                        f"[-] Failed login attempt {self.attempts}/{len(self.passwords)}: {password}"
                    )
                    ...
                    logger.warning(
                        "Target may be down or blocking connections. Stopping attack simulation."
                    )
                    break
                    ...
                    # Other error
                    logger.error(
                        f"[!] Unexpected error during login attempt with password: {password}"
                    )
...
def parse_arguments() -> argparse.Namespace:
    ...
    # Required arguments
    parser.add_argument(
        "-t", "--target", dest="target_ip", required=True, help="Target SSH server IP address"
    )
    ...
    parser.add_argument(
        "-u", "--username", dest="username", required=True, help="Username to authenticate with"
    )

    # Password sources (mutually exclusive)
    password_group = parser.add_mutually_exclusive_group(required=True)
    password_group.add_argument(
        "-w", "--passwords", dest="passwords", help="Comma-separated list of passwords to try"
    )
    password_group.add_argument(
        "-P",
        "--password-file",
