...
class SynFlood(AttackBase):
    ...
    def add_arguments(self, parser: argparse.ArgumentParser) -> None:
        ...
        behavior_group.add_argument(
            "--payload-size", type=int, default=0, help="Additional payload size in bytes"
        )
        behavior_group.add_argument(
            "--window-size", type=int, default=8192, help="TCP window size"
        )
        behavior_group.add_argument(
            "--flags", default="S", help="TCP flags (S=SYN, A=ACK, F=FIN, R=RST, P=PSH)"
        )
        ...
        if args.port_range:
            try:
                ...
                if start_port >= end_port:
                    logger.error("Invalid port range: start port must be less than end port")
                    return False
            except ValueError:
                logger.error("Invalid port range format. Use start-end (e.g., 80-85)")
                return False
        else:
