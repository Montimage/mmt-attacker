...
class HttpDos(AttackBase):
    ...
    def add_arguments(self, parser: argparse.ArgumentParser) -> None:
        ...
        target_group.add_argument(
            "--method", default="GET", help="HTTP method (GET, POST, HEAD, PUT, DELETE, OPTIONS)"
        )
        target_group.add_argument("--path", default="/", help="Target path (default: /)")
        target_group.add_argument(
            "--port", type=int, help="Target port (default: 80/443 based on scheme)"
        )
        ...
        behavior_group.add_argument(
            "--count", type=int, default=1000, help="Number of requests to send"
        )
        behavior_group.add_argument(
            "--interval", type=float, default=0.1, help="Time interval between requests (seconds)"
        )
        behavior_group.add_argument(
            "--ramp-up", type=int, help="Gradually increase request rate over seconds"
        )
        behavior_group.add_argument(
            "--random-path", action="store_true", help="Randomize path for each request"
        )
        behavior_group.add_argument(
            "--random-query", action="store_true", help="Add random query parameters"
        )
        ...
        monitor_group.add_argument(
            "--success-codes", help="HTTP codes to consider success (default: 200-299)"
        )
        monitor_group.add_argument(
            "--verify-success",
            action="store_true",
    ...
    def validate(self, args: argparse.Namespace) -> bool:
        ...
        if args.data_file and not self.validator.validate_file_path(
            args.data_file, check_readable=True
        ):
        ...
        if args.success_codes:
            try:
                ...
                logger.error(
                    f"Invalid success codes: {args.success_codes}. Use format: 200 or 200-299"
                )
                return False
